...
FROM s3.tmp.`big_files/T_DECIMAL_BIG` decimal_big
JOIN `s3.tmp`.`big_files/T_DECIMAL_BIG_ZERO_PREC` decimal_big_zero_prec
  ON decimal_big.ID = decimal_big_zero_prec.ID
  AND decimal_big.ID > 2
JOIN s3.tmp.`big_files/T_DECIMAL_BIG_ZERO_SCALE` decimal_big_zero_scale
...
FROM `s3.tmp`.`big_files/decimal_big.tsv` decimal_big_tsv
JOIN `s3`.`tmp`.`big_files/decimal_big_zero_scale.tsv` decimal_big_zero_scale_tsv
...
JOIN s3.tmp.`big_files/T_DECIMAL_BIG_ZERO_PREC` second_decimal_big_zero_prec
